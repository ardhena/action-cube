defmodule ActionCubeWeb.SnakeLiveTest do
  ...
  test "renders initial board view", %{conn: conn} do
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"15-15\"></td>"
  ...
  test "processes tick when message is received", %{conn: conn} do
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"14-15\"></td>"
  end
  ...
  test "ends game when snake goes into wall", %{conn: conn} do
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"10-15\"></td>"
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"5-15\"></td>"
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"0-15\"></td>"
    assert render(live_view) =~ "Game is running"
  ...
  test "changes snake direction on key press (wsad)", %{conn: conn} do
    ...
    assert render(live_view) =~ "<td class=\"empty\" id=\"15-15\"></td>"
    assert render(live_view) =~ "<td class=\"snake\" id=\"15-14\"></td>"
    ...
    assert render(live_view) =~ "<td class=\"snake\" id=\"14-14\"></td>"
